...
function TeacherThematic(props) {
  ...
  const getListTeacherThematic = (isLoading = true, callback) => {
    isLoading && setLoading(true)
    reportsApi
      .getListTeacherThematic(BrowserHelpers.getRequestParamsSchools(filters))
      .then(({ data }) => {
        if (data.isRight) {
      ...
      <div className="bg-white rounded">
        <div className="px-20px py-15px border-bottom border-gray-200 d-flex align-items-center justify-content-between">
          <div className="fw-500 font-size-lg">
            Danh sách Giáo viên - Tiết chuyên đề
          </div>
        </div>
...
export default TeacherThematic
